from layers.DynamicGCN import *
from layers.TCN import *
        ...
        for block_id in range(self.n_blocks):
            self.blocks.append(
                Block(block_id, self.total_t_len, config.kernel_set, config.dilation_exponential, config.sumba_layers, config.residual_channels, config.conv_channels, config.gcn_depth,
                 config.M, config.dimension, config.skip_channels, config.num_nodes, layer_norm_affline, propalpha, config.dropout_MTGNN,config.D,config.LowRank))
